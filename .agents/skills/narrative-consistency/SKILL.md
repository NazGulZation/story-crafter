...
description: >-
  Comprehensive framework and methodology for auditing narrative consistency, detecting and eliminating plot holes,
  enforcing ironclad causality ("Therefore / But"), verifying timeline and information asymmetry integrity,
  and preventing unseeded deus ex machina resolutions in creative writing projects.
  Also audits explicit, detailed, arousing NSFW/erotica, protection-asset tracking, stamina timeline, and arousal-continuity coherence.
---
...
```mermaid
flowchart TD
    A[Step 1: Epistemic & Asset Inventory] --> B[Step 2: Causal Trace & Friction Test]
    B --> C[Step 3: Psychological & Physical Sanity Check]
    C --> D{Plot Holes Found?}
    D -- Yes --> E[Execute Causal Repair Protocol]
    D -- No --> F[Verify Final Text Against Workspace Constraints]
```
...
- [ ] **Protection-asset count**: Condoms/lube/cleanup seeded 1-2 scenes early; used count equals cleanup count (e.g. 4 used = 4 bagged); never conjured mid-climax.
...
- [ ] **No reset across break**: Opening state of Ch N+1 equals closing state of Ch N — same hardness/wetness/soreness, same fluids/smell/stickiness, same clothing positions (rucked vs. off vs. on floor), same location mess (damp spot, spilled lube, condom wrapper). Flag instant re-hardening without work, vanishing fluids, mysterious redressing, or fresh-sheet amnesia as FAIL.
